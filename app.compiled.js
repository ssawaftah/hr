...
function manageableDepartmentIds(employeeId = currentUser().id) {
  const roleDepartments = rolesForEmployee(employeeId).filter((role) => role.permissions && role.permissions.manageDepartment).map((role) => role.departmentId).filter(Boolean);
  const managedDepartments = Object.entries(state.departments).filter(([, department]) => department.managerId === employeeId || department.assistantId === employeeId).map(([id]) => id);
  return [.../* @__PURE__ */ new Set([...roleDepartments, ...managedDepartments])];
}
